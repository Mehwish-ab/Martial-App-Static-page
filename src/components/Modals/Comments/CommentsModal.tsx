...
const CommentsModal: React.FC<commentsModalProps> = ({
    ...
    const showReplyToggler = (id: number) => {
        const updatedComments = commentsData.comments.map(
    ...
    const getCommentsPromise = async () => {
        try {
            ...
            setCommentData({
                ...data.results,
                comments: data.results.comments.map(
                    (comment: commentTypes) => ({
                        ...comment,
                        showReply: false,
                        showEdit: false,
    ...
    const deleteCommentFilter = (commentId: number) => {
        ...
        setCommentData({
            ...oldCommentsData,
            comments: oldCommentsData.comments.filter(
                ({ id }) => id !== commentId
            ),
        })
    }
            ...
            comments: oldCommentsData.comments.map((comment: any) => {
                if (comment.id === commentId) {
                    const newChildComments = comment.childrenComments.filter(
                        (childComment: any) =>
                            childComment.id !== childCommentId
                    )
                    return {
                        ...comment,
